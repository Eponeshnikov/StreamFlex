    ...
    def load_plugins(self):
        ...
        for entry in self.plugins_dir.iterdir():
            ...
            if not module_path.exists():
                self.logger.warning(
                    "Plugin directory missing plugin.py", plugin=plugin_name
                )
                continue
            ...
            try:
                ...
                for attr_name in dir(module):
                    ...
                    if (
                        ...
                        self.logger.debug(
                            f"Found Plugin subclass {attr.__name__}",
    ...
    def install_plugin(self, repo_url: str) -> bool:
        ...
        try:
            ...
            self.logger.opt(exception=True).critical(
                "Plugin installation failed", repo_url=repo_url, error=str(e)
            )
            if plugin_dir.exists():
                self.logger.debug(
                    "Cleaning up failed installation", path=str(plugin_dir)
                )
                import shutil
